...
{
	/* ponytail: on this OEM CN66XX SNIC10E the host-driven CIU_SOFT_RST has
	 * no effect (verified over the serial console: the Octeon never reboots),
	 * so the stock reset+scratch-verify always fails. We instead put the card
	 * at the u-boot prompt ourselves (host-boot mode), which is the same state
	 * a successful reset would leave it in, then let the normal driver flow
	 * (wait_for_bootloader -> console bootcmd -> download firmware) proceed.
	 * So skip the reset write and report success.
	 */
	octeon_write_csr64(oct, CN6XXX_WIN_WR_MASK_REG, 0xFF);
	dev_info(&oct->pci_dev->dev,
		 "soft_reset: SKIPPED (host-boot mode; CIU_SOFT_RST is a no-op on this card)\n");
	return 0;
}
