...
/* ponytail: attach to a NIC firmware already booted on the card (OEM CN66XX
 * whose host soft-reset is broken). 1 = skip reset/DDR/bootloader/download.
 */
static int preloaded;
module_param(preloaded, int, 0644);
MODULE_PARM_DESC(preloaded,
		 "1 = firmware already running on card; skip soft-reset and firmware download (for OEM CN66XX where host soft-reset fails).");
...
static void nic_starter(struct work_struct *work)
{
	...
	/* ponytail: on this OEM CN66XX the legacy INTx interrupt is not reliably
	 * delivered to the host, so the firmware's CORE_DRV_ACTIVE packet can sit
	 * unprocessed in the output queue and the CORE_OK state is never reached.
	 * Poll the output queues here so the dispatch (octeon_core_drv_init) runs
	 * without depending on an interrupt.
	 */
	if (atomic_read(&oct->status) < OCT_DEV_CORE_OK) {
		...
		for (q = 0; q < oct->num_oqs; q++) {
			if (oct->droq[q])
				octeon_droq_process_packets(oct, oct->droq[q],
							    MAX_PACKET_BUDGET);
		}
	}
...
static int octeon_device_init(struct octeon_device *octeon_dev)
{
	...
	/* ponytail: preloaded override. On OEM CN66XX SNIC10E the host soft-reset
	 * does not work, so we boot the NIC firmware on the card ourselves
	 * (u-boot loady + bootoct) and tell the driver to attach to it instead
	 * of resetting + downloading. preloaded=1 forces FW_IS_PRELOADED, which
	 * skips soft_reset, DDR wait, bootloader handshake and firmware download.
	 */
	if (preloaded)
		atomic_cmpxchg(octeon_dev->adapter_fw_state,
			       FW_NEEDS_TO_BE_LOADED, FW_IS_PRELOADED);
